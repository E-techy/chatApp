...
sendButton.addEventListener("click",()=>{
    ...
    if (chatTypePrivate.checked) {
        ...
        socket.emit("privateMessageToServer",message,joinedRoom)
        console.log("Message Sent");
        displayMessage(message,"Me")
    }
...
socket.on("publicMessage",(message,socketId)=>{
    if (message=="This user is disconnected from the Chat.") {
        var noOfUsers=document.getElementById("noOfUsers")
        noOfUsers.innerText=Number(noOfUsers.innerText)-1;
    }
    var chatTypePrivate=document.getElementById("privateChat");
    if (chatTypePrivate.checked) {
        return;
...
document.getElementById('file').addEventListener('change', function() {
    ...
    reader.onload = function() {
      ...
      if (chatTypePrivate.checked) {
        socket.emit('privateImage',bytes,socket.id,joinedRoom);
      }
      else{
      socket.emit('image', bytes,socket.id);
      }
    };
...
//Function to display the full size image of the images the in chat which are in smaller size;
